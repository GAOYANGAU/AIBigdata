...
"""
打砖块游戏: 训练脚本
该脚本中相对 train.py 脚本减少了图像展示和记录日志部分, 能加快学习过程, 适合在命令行使用
...
flags.DEFINE_integer('resized_width', 84)
flags.DEFINE_integer('resized_height', 84)
flags.DEFINE_float('learning_rate', 0.0001)
flags.DEFINE_integer('num_concurrent', 1)
flags.DEFINE_string('experiment', 'dqn_breakout')
flags.DEFINE_string('checkpoint_dir', 'checkpoints')
flags.DEFINE_integer('tmax', 10000000)
flags.DEFINE_integer('anneal_epsilon_timesteps', 1000000)
...
def actor_learner_thread(thread_id, env, session, graph_ops, num_actions, summary_ops, saver):
    ...
    summary_placeholders, update_ops = summary_ops
