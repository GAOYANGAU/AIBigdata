...
class Tacotron():
	...
	def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, targets_lengths=None,
			global_step=None, is_training=False, split_infos=None):
		"""
		...
		for i in range(hp.tacotron_num_gpus):
			with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
				with tf.variable_scope('inference') as scope:
					...
					if is_training:
						self.helper = TacoTrainingHelper(batch_size, tower_mel_targets[i], hp, global_step)
					else:
		...
		for i in range(hp.tacotron_num_gpus):
			with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
				with tf.variable_scope('loss') as scope:
					...
					tower_loss = before + after + stop_token_loss + regularization
