...
class Feeder:
	...
	def __init__(self, coordinator, metadata_filename, hparams):
		...
		with tf.device('/cpu:0'):
			...
			queue = tf.FIFOQueue(8, [tf.int32, tf.int32, tf.float32, tf.float32, tf.int32, tf.int32], name='input_queue')
			self._enqueue_op = queue.enqueue(self._placeholders)
			self.inputs, self.input_lengths, self.mel_targets, self.token_targets, self.targets_lengths, self.split_infos = queue.dequeue()
			...
			eval_queue = tf.FIFOQueue(1, [tf.int32, tf.int32, tf.float32, tf.float32, tf.int32, tf.int32], name='eval_queue')
			self._eval_enqueue_op = eval_queue.enqueue(self._placeholders)
			self.eval_inputs, self.eval_input_lengths, self.eval_mel_targets, self.eval_token_targets, \
				self.eval_targets_lengths, self.eval_split_infos = eval_queue.dequeue()
	...
	def _prepare_batch(self, batches, outputs_per_step):
		...
		return (inputs, input_lengths, mel_targets, token_targets, targets_lengths, split_infos)
